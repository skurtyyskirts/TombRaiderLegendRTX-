/*
 * Wrapped IDirect3D9 - intercepts CreateDevice to return a traced device.
 * All other methods forward to the real IDirect3D9.
 *
 * Adapted from the FFP template d3d9_wrapper.c for the trace proxy.
 */
...
#define D3D9_RELAY(SLOT, ARGC, ...) /* defined below per-method */
    ...
    void *pPresentParams, void **ppDevice)
{
    ...
    {
        TracedDevice *traced = TracedDevice_Create(pRealDevice);
        if (!traced) {
            log_str("  ERROR: Failed to create traced device\r\n");
            *ppDevice = pRealDevice;
            return hr;
        }
        log_hex("  Traced device: ", (unsigned int)(size_t)traced);
        *ppDevice = (void*)traced;
    }
...
WrappedD3D9* WrappedD3D9_Create(void *pRealD3D9) {
    ...
    log_hex("WrappedD3D9 created at ", (unsigned int)(size_t)w);
    return w;
}
