...
namespace comp
{
	...
	void imgui::tab_ffp()
	{
		...
		// Register layout — LA Noire resolves VS registers dynamically per
		// shader, so matrices come from game-specific hooks (see
		// comp/game/hooks.cpp) rather than a fixed register range.
		if (ImGui::CollapsingHeader("Register Layout"))
		{
			ImGui::TextUnformatted("View / Proj / World : captured via LA Noire hooks");
			ImGui::TextUnformatted("  - SetCameraMatrices @ 0x00D5E910");
			ImGui::TextUnformatted("  - SetShaderConstant @ 0x00D60AB0 + renderer+0x284 (ModelMatrix)");
			ImGui::TextUnformatted("  - renderer+0x5C0 per-frame (Projection)");
		...
		// Matrix values (sourced from LA Noire hooks, not VS constant registers)
		if (ImGui::CollapsingHeader("Matrices"))
		{
			auto show_matrix = [&](const char* name, const float* m, bool valid)
			{
				if (!valid || !shared::common::ffp_state::mat4_is_interesting(m))
					return;
			...
			show_matrix("View",       ffp.lanoire_view(),  ffp.lanoire_view_valid());
			show_matrix("Projection", ffp.lanoire_proj(),  ffp.lanoire_proj_valid());
			show_matrix("World",      ffp.lanoire_world(), /* always show last capture */ true);
		}
