...
namespace comp
{
	void on_begin_scene_cb()
	{
		...
		// LA Noire stores its projection matrix inline in the renderer object
		// (renderer + 0x5C0) rather than exposing it through a named shader
		// constant. Pull it once per frame so ffp_state has a live Projection
		// paired with the View captured by SetCameraMatrices_hook.
		if (const float* proj = game::get_projection_matrix_ptr())
			shared::common::ffp_state::get().set_lanoire_proj(proj);
	}
