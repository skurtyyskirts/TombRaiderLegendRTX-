...
namespace shared::common
{
	...
	public:
		...
		// LA Noire resolves VS constant registers dynamically per-shader, so the
		// default register-range capture in on_set_vs_const_f is unreliable. The
		// game-specific hooks in comp/game/ call these setters directly with the
		// matrices in their source form (column-major, 4x4 = 16 floats), captured
		// *before* the game concatenates them into ModelViewProjectionMatrix.

		void set_lanoire_view(const float* view_col_major);
		void set_lanoire_proj(const float* proj_col_major);
		void set_lanoire_world(const float* world_col_major);

		const float* lanoire_view()  const { return view_matrix_; }
		const float* lanoire_proj()  const { return proj_matrix_; }
		const float* lanoire_world() const { return world_matrix_; }
		bool lanoire_view_valid()  const { return view_valid_; }
		bool lanoire_proj_valid()  const { return proj_valid_; }
		...
		// VS constant register layout (skinning-only on LA Noire — the game's
		// dynamic per-shader register assignment makes view/proj/world ranges
		// meaningless, so those have been removed).
	...
	private:
		...
		// Transform capture (LA Noire path). view_valid_/proj_valid_ gate
		// view_proj_valid() for draw routing. view_dirty_/proj_dirty_/world_dirty_
		// avoid redundant SetTransform calls.
		float view_matrix_[16] = {};
		float proj_matrix_[16] = {};
		float world_matrix_[16] = {};
		bool view_valid_ = false;
		bool proj_valid_ = false;
		...
		// Skinning register layout (unused until [Skinning] Enabled=1 — defaults
		// are placeholders; run find_skinning.py on LANoire.exe and update
		// before enabling skinning).
