...
:: Find Visual Studio. VS 2022+ 64-bit installer places everything under
:: %ProgramFiles% (no ProgramFiles(x86) needed).
set "VCVARS="
for %%p in (
    "%ProgramFiles%\Microsoft Visual Studio\18\Community\VC\Auxiliary\Build\vcvarsall.bat"
    "%ProgramFiles%\Microsoft Visual Studio\18\Professional\VC\Auxiliary\Build\vcvarsall.bat"
    "%ProgramFiles%\Microsoft Visual Studio\18\Enterprise\VC\Auxiliary\Build\vcvarsall.bat"
...
if "%VCVARS%"=="" (
    echo ERROR: Visual Studio with C++ x86 build tools not found under "%ProgramFiles%"
    exit /b 1
)
